...
!===============================================================================
!===============================================================================
   v_b = 5*duex!dble(0.01d0*uex_max)
   open (3,file='gauss.bin',status='unknown',form='unformatted')
   write (3) v_b
   close(3)
!===============================================================================
...
!     
     ...
     do 12 iu = 1, nuex
       ...
       temp3 = 0
       !do 13 iuu = 1,nuex
       !   itemp2 = INT(1+(uex(iu)-uex(iuu)+uex_max)/(uex(2)-uex(1)))
       !   if(itemp2 < 1 .or. itemp2>nuex) then 
       !      temp3 = temp3 + 0
       !   else
       !      temp3 = temp3+Gaussian(uex(itemp2),Vx,v_b)*Gaussian(uex(iuu),Vx,Vthe)*uex(iuu)**2*(1.d0+alpha*dcos(ak*x(i)))*(uex(2)-uex(1))
       !   end if
       !13 continue       
